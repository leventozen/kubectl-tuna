...
The GIF is meant to look like a real terminal session:
  1. type the command
...
PROMPT = "levent@mac ~/code/kdiag"
CMD = "kubectl diag inspect service payment -n kdiag-demo"
...
MARGIN_Y = 52
LINE_H = 17
CHAR_W = 9  # Menlo 14 ≈ this; refined per glyph when drawing
...
def wrap_segments(
    ...
    """Wrap a colored line into multiple display rows."""
...
def build_screen_rows(
    ...
    rows.append(
        [
            ...
            ("\n", C_DEFAULT),  # marker unused; we split manually
        ]
    )
    # Actually keep prompt + command on two visual concepts:
    rows = [
        [(PROMPT, C_PROMPT_PATH)],
        [("$ ", C_PROMPT_DOLLAR), (cmd_typed, C_CMD)],
    ]
    if cmd_typed == CMD:
        # blank line after command, then real report
        rows.append([])
        for line in output_lines:
            segs = colorize_line(line)
            wrapped = wrap_segments(draw, segs, f, W - 2 * MARGIN_X)
            rows.extend(wrapped)
    return rows
...
def render(
    ...
    # Build rows without the fake newline hack
    rows: list[list[tuple[str, tuple[int, int, int]]]] = [
        [(PROMPT, C_PROMPT_PATH)],
        [("$ ", C_PROMPT_DOLLAR), (cmd_typed, C_CMD)],
    ]
...
def main() -> None:
    ...
    # 2) Type the real command
    for i in range(1, len(CMD) + 1):
        frames.append(render(f, CMD[:i], [], cursor=True))
        durations.append(28 if CMD[i - 1] != " " else 50)
